<template>
  <div id="Masks"></div>
  <div id="verify">
    <div class="slider-verification" ref="sliderContainer">
      <div class="slider-left-bg" :style="sliderLeftWidthStyle(slideWidth)"></div>
      <div class="prompt-text" :style="verifySuccess ? props.promptTextStyleSuccess : {}">
        {{ verifySuccess ? verifyMsg(props.verified) : '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;向右拖动滑块进行验证' }}

        <!-- 下面v-if是为了判断合适使用加载图标，即当 滑块到底 且 人机验证结果未出来时 -->
        <Loading v-if="verifySuccess && props.verified == null" class="isLoading" />
      </div>
      <div ref="slider" @mousedown="mouseDownHandler($event)" :class="{ 'slider-verify-success': verifySuccess }"
        class="slider" :style="sliderToLeftStyle(slideWidth)"></div>
    </div>
  </div>
</template>

<script setup>
// vue基本
import { defineComponent, ref, onMounted, defineEmits, computed, getCurrentInstance } from 'vue';

// components组件

// 本地库
import { sliderToLeftStyle, sliderLeftWidthStyle, verifyMsg } from './js/verifyDiv_js'

// 第三方库
import axios from 'axios'


let sliderState = ref(false)
let verifySuccess = ref(false)
let beginClientX = ref(0)
let maxWidth = ref(0)
const slideWidth = ref(0)
let mouseAxios = []

const { emit, proxy, ctx } = getCurrentInstance()
const _this = ctx

const props = defineProps({
  promptTextStyleSuccess: {
    type: Object,
    default: {
      color: '#ffffff'
    }
  },
  /**
   * 验证成功需要拖动距离倍数，倍数越大验证成功需要拖动的距离越大
   */
  multiple: {
    type: Number,
    default: 2
  },
  verified: {
    type: Boolean
  }
});

const mouseMoveHandler = (e) => {
  if (sliderState.value) {
    let width = (e.clientX - beginClientX.value) / props.multiple;

    if (width > 0 && width <= maxWidth.value) {
      slideWidth.value = width
    } else if (width > maxWidth.value) {
      slideWidth.value = maxWidth.value
      verifySuccessFun();
    }
  }
}
const mouseUpHandler = (e) => {
  sliderState.value = false;
  let width = (e.clientX - beginClientX.value) / props.multiple;
  if (width < maxWidth.value) {
    slideWidth.value = 0
  }
}
const verifySuccessFun = () => {
  verifySuccess.value = true
  emit('statusChange', true)
  document.getElementsByTagName('html')[0].removeEventListener('mousemove', mouseMoveHandler);
  document.getElementsByTagName('html')[0].removeEventListener('mouseup', mouseUpHandler);
  slideWidth.value = maxWidth.value
}

// 实时记录鼠标坐标
var mouseAxiosInfo
var mouseAxiosList = []
var arCoords = []

document.getElementsByTagName('html')[0].onmousemove = function (event) {
  if (sliderState.value && !verifySuccess.value) {
    var e = window.event || e;
    arCoords = [e.clientX, e.clientY];

  }
}

// 获取滑动时鼠标坐标
const getMouseAxios = function () {
  if (sliderState.value && !verifySuccess.value) {
    mouseAxiosList.push(arCoords)


  } else {
    mouseAxiosInfo = { mouseAxiosList }
    if (mouseAxiosList.length !== 0 && verifySuccess.value) {
      mouseAxiosInfo = { mouseAxiosList }
      postMouseAxios({
        'type': 'verify',
        'data': mouseAxiosInfo
      })
      // context.emit('update:verified', mouseAxiosInfo)
    }
    mouseAxiosList = []
  }
  setTimeout(getMouseAxios, 100)
}
getMouseAxios()

const postMouseAxios = (postData) => {
  const customConfig = {
    headers: {
      'Content-Type': 'application/json'
    }
  };
  axios.post('/login', postData, customConfig)
    .then((res) => {
      if (res.status == '200') {
        if (res.data.code == 200) {
          emit('update:verified', true)
        } else if (res.data.code == 300) {
          emit('update:verified', null)
        } else {
          emit('update:verified', false)
        }
      } else {

      }
    })

}


const slider = ref(null)
const sliderContainer = ref(null)
onMounted(() => {
  maxWidth.value = sliderContainer.value.offsetWidth - slider.value.offsetWidth;
  addListener()
})
const addListener = () => {
  document.getElementsByTagName('html')[0].addEventListener('mousemove', mouseMoveHandler);
  document.getElementsByTagName('html')[0].addEventListener('mouseup', mouseUpHandler)
}

const reset = () => { // 重置
  addListener()
  emit('statusChange', false)
  beginClientX.value = 0
  sliderState.value = false
  verifySuccess.value = false
  slideWidth.value = 0
}

const mouseDownHandler = (e) => { // 鼠标在滑块上按下事件
  if (!verifySuccess.value) {
    sliderState.value = true;
    beginClientX.value = e.clientX;
  }
}

</script>

<style lang="scss">
$div-width: 200px;
$div-height: 40px;
#Masks{
  backdrop-filter: blur(5px);
  position: absolute;
  top: 0;
  left: 0;
  bottom: 0;
  right: 0;
  translate: 0px 0px 499px;
}

#verify {
  position: absolute;
  top: 0;
  left: 0;
  right: 0;
  bottom: 0;
  margin: auto;
  translate: 0 0 500px;

}

.isLoading {
  animation: rotate 2s linear infinite;
  width: 1em;
  height: 0.85em;
  margin-right: 8px
}

#verify {
  width: $div-width;
  height: $div-height;
  // translate: 0 0 500px;
  position: absolute;
  top: 0;
  left: 0;
  right: 0;
  bottom: 0;
  margin: auto;
}

.slider-verification {
  position: relative;
  background-color: #055265;
  width: 100%;
  height: 100%;
  line-height: 48px;
  text-align: center;
  user-select: none;

  border-radius: calc($div-height/2);
}

.slider {
  position: absolute;
  top: 0;
  width: 42px;
  height: 100%;
  cursor: move;
  background: rgb(0, 157, 177) url("data:image/png;base64,") no-repeat center;
  background-size: 30px 30px;
  border-radius: calc($div-height/2);
}

.slider-verify-success {
  background: #00a246 url("data:image/png;base64,") no-repeat center;
  background-size: cover;

  border-radius: calc($div-height/2);
}

//滑块滑过区域的样式
.slider-left-bg {
  background-color: rgba(0, 156, 177, 0.807);
  width: 0;
  height: 100%;

  border-top-left-radius: calc($div-height/2);
  border-bottom-left-radius: calc($div-height/2);
}

//提示文本样式
.prompt-text {
  position: absolute;
  top: 0;
  width: 100%;
  height: 100%;
  text-align: center;
  font-size: 14px;
  line-height: 1;
  padding: 13px;
  color: #ffffff;
  opacity: 0.7;
}
</style>